...
const ORD_EVERY = Number(process.env.PAP_ORD_EVERY || 0); // 0 = no ordinary sample
const DENSE_ONLY = process.env.PAP_DENSE_ONLY === '1';
...
const dense = labeled.filter((i) => i.actual >= 40);
const ordSample = ORD_EVERY > 0
  ? labeled.filter((i) => i.actual < 40).filter((_, idx) => idx % ORD_EVERY === 0)
  : [];
const CORPUS = DENSE_ONLY ? dense : [...dense, ...ordSample]; // anchors already covered in pass 1
out(`[pap1671-capture-probe] target=${TARGET} corpus=${CORPUS.length} (dense=${dense.length} ordSample=${ordSample.length}) anchor=silhouette-walk`);
...
for (const item of CORPUS) {
  try {
    ...
    rows.push({
      ...
      targetPx: TARGET,
      censusLegacy: legacy.some((p) => p && Math.abs(p.tc - item.actual) <= 1 && p.rel >= 0.04),
      censusRaw2048: raw2048.some((p) => p && Math.abs(p.tc - item.actual) <= 1 && p.rel >= 0.04),
      censusTp2048: tp2048.some((p) => p && Math.abs(p.tc - item.actual) <= 1 && p.rel >= 0.04),
  ...
  if (rows.length % 25 === 0 || rows.length === CORPUS.length) {
    out(`[pap1671-capture-probe] ${rows.length}/${CORPUS.length} ${((Date.now() - t0) / 1000).toFixed(0)}s`);
    try { fs.writeFileSync(path.join(DEBUG_DIR, `probe_t${TARGET}_rows.json`), JSON.stringify(rows, null, 1)); } catch { }
  }
}
fs.writeFileSync(path.join(DEBUG_DIR, `probe_t${TARGET}_rows.json`), JSON.stringify(rows, null, 1));
out(`[pap1671-capture-probe] done rows=${rows.length} elapsed=${((Date.now() - t0) / 1000).toFixed(0)}s`);
