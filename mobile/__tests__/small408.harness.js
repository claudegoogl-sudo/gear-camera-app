/**
 * PAP-408 diagnostic harness — small-gear (11-15T) 72% → 90% investigation.
 *
 * Runs current gearCounter on the 5 labeled misses from the b85/86/88
 * window (per PAP-408 evidence table) plus the 13 hits as control.
 * Prints per-report diagnostic breakdown: peakTc, fft90tc, bcTc, bcPeaks,
 * opTc, gearR, contourR, methodUsed, etc.  Used only to classify failure
 * modes; not a Jest assertion.
 */
...
function loadSmallGearWindow() {
  // 11-15T reports from builds 85-88.
  const rows = [];
  for (const dir of fs.readdirSync(REPORTS).filter(d => d.startsWith('report_')).sort()) {
    ...
    if (!actual || actual < 11 || actual > 15) continue;
    const build = parseBuild(meta.build);
    if (build < 85 || build > 88) continue;
...
describe('PAP-408 small-gear diagnostic', () => {
  jest.setTimeout(30 * 60 * 1000);
  test('b85-88 11-15T labeled reports — per-report diagnostic breakdown', () => {
    const { countTeethFromRgba } = require('../src/algorithm/gearCounter');
    const rows = loadSmallGearWindow();
    console.log(`\n=== PAP-408 small-gear (11-15T) b85-88 window: n=${rows.length} ===\n`);
    let hits = 0;
    const perClass = {};
    for (const r of rows) {
