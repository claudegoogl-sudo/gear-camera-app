/**
 * PAP-406 diagnostic harness — large-gear (22-28T) 0% → investigate.
 *
 * Runs current gearCounter on all labeled 22-28T reports from builds 83-89
 * and prints per-report diagnostic breakdown including FFT bin magnitudes
 * in [10-32] so we can tell mode-1 (center fail) from mode-2 (FFT ceiling)
 * from sub-harmonic aliasing.
 */
...
describe('PAP-406 large-gear diagnostic', () => {
  jest.setTimeout(30 * 60 * 1000);
  test('b83-89 22-28T labeled reports — per-report diagnostic breakdown', () => {
    const { countTeethFromRgba } = require('../src/algorithm/gearCounter');
    const rows = loadLargeGearWindow();
    console.log(`\n=== PAP-406 large-gear (22-28T) b83-89 window: n=${rows.length} ===\n`);
    let hits = 0;
    const perClass = {};
    for (const r of rows) {
