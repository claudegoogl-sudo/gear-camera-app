...
const DEBUG_DIR = path.join(ROOT, 'debug-reports', 'pap1862_fp5_b151_session_2026-09-10');
...
const sweep = JSON.parse(fs.readFileSync(path.join(DEBUG_DIR, 'threshold_sweep_rows.json'), 'utf8'));
const bandByStamp = new Map(sweep.filter((r) => !r.error).map((r) => [r.stamp, r]));
...
out(`[pap1862-audit] corpus=${labeled.length} + anchors=${ANCHORS.length} at aabd380 (gate off)`);
const rows = [];
const t0 = Date.now();
for (const item of [...ANCHORS, ...labeled]) {
  ...
  rows.push({
    ...
    runtime,
    gateBand: sw ? sw.fraction < 0.50 : null,
    gateFraction: sw ? sw.fraction : null,
  });
  if (rows.length % 25 === 0 || rows.length === labeled.length + ANCHORS.length) {
    out(`[pap1862-audit] ${rows.length}/${labeled.length + ANCHORS.length} ${((Date.now() - t0) / 1000).toFixed(0)}s`);
  }
  if (rows.length % 100 === 0) {
    try { fs.writeFileSync(path.join(DEBUG_DIR, 'post_fix_corpus_audit_rows.json'), JSON.stringify(rows, null, 1)); } catch { }
  }
}
fs.mkdirSync(DEBUG_DIR, { recursive: true });
fs.writeFileSync(path.join(DEBUG_DIR, 'post_fix_corpus_audit_rows.json'), JSON.stringify(rows, null, 1));
out(`[pap1862-audit] done rows=${rows.length} elapsed=${((Date.now() - t0) / 1000).toFixed(0)}s`);
