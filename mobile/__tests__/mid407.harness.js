/**
 * PAP-407 diagnostic harness — mid-gear (16-21T) 46% → target ≥90%.
 *
 * Runs current gearCounter on all labeled 16-21T reports from builds 83-89
 * and prints per-report diagnostic breakdown mirroring large406.harness.js.
 */
...
function loadMidGearWindow() {
  ...
  for (const dir of fs.readdirSync(REPORTS).filter(d => d.startsWith('report_')).sort()) {
    ...
    if (!actual || actual < 16 || actual > 21) continue;
...
describe('PAP-407 mid-gear diagnostic', () => {
  jest.setTimeout(30 * 60 * 1000);
  test('b83-89 16-21T labeled reports — per-report diagnostic breakdown', () => {
    const { countTeethFromRgba } = require('../src/algorithm/gearCounter');
    const rows = loadMidGearWindow();
    console.log(`\n=== PAP-407 mid-gear (16-21T) b83-89 window: n=${rows.length} ===\n`);
    let hits = 0;
    const perClass = {};
    for (const r of rows) {
