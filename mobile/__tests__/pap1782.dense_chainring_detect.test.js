...
// Mock expo dependencies that gearCounter imports
...
describe('PAP-1782: D3 Dense Chainring Detection', () => {
  ...
  test('estimateInnerRadius: dense chainring should return small radius', () => {
    ...
    console.log('DENSE fraction:', fraction); expect(fraction).toBeLessThan(0.50); // Should be well below 0.50
  ...
  test('estimateInnerRadius: small gear should return large radius', () => {
    ...
    const fraction = innerR / contourRadius;
    console.log('SMALL fraction:', fraction); expect(fraction).toBeGreaterThan(0.20); // Should be above 0.50
  ...
  test('estimateInnerRadius: mid gear should return medium radius', () => {
    ...
    const fraction = innerR / contourRadius;
    console.log('MID fraction:', fraction); expect(fraction).toBeGreaterThan(0.20);
  ...
  test('checkDenseChainringRegime: small gear synthetic returns valid result', () => {
    const gray = createTestGray(width, height, 'small-gear');
    const result = checkDenseChainringRegime(gray, cx, cy, contourRadius, contourRadius, width, height);

    // Synthetic data doesn't perfectly match real gears, but result should be valid
    expect(result.isDense).toBeDefined();
    expect(result.confidence).toBeGreaterThanOrEqual(0);
    expect(result.confidence).toBeLessThanOrEqual(1);
    expect(result.fraction).toBeGreaterThan(0);
  });

  test('checkDenseChainringRegime: mid gear synthetic returns valid result', () => {
    const gray = createTestGray(width, height, 'mid-gear');
    const result = checkDenseChainringRegime(gray, cx, cy, contourRadius, contourRadius, width, height);
