...
**Build**: **b155 — LIVE AND QA-VERIFIED** (tag `b155` → `e7fedc3` exact; APK sha256 `c5b6cb30dd9b…43cf` matches; bundle markers flashControlToggled/torchProp/pap1872-dense-chainring/frameProcessorTimeout/imu-only-panel all present; PAP-1890 lane done, QA ack bd4f2630). Main HEAD incl. flash control `114a496` + `torchEngaged` telemetry fix `6d26228` + PAP-1879 camera diagnostics. Operator shot list posted (b9001000, [[operator-deliver]]). Rationale: operator's 17:40Z b153 attempt failed ALL captures in the dark (no flash control) — b155 adds the illumination lever so a dark burst can't void the booking. `gearCounter.js` identical to b153's `3d7770b` (empty diff) → corpus pre-flight carries unchanged. Fallback if b155 slips: b153 `06befc3` (https://github.com/claudegoogl-sudo/gear-camera-app/releases/tag/b153); original b153 header text below for provenance. --- **b153 (superseded session target)**: release tag/b153 (main code @ `3d7770b`, build-info `06befc3`); calibrated pap1872 G1-G4 dense-abstain gate + cannot-count panel
